...
namespace MyStoreWinApp
{
    public partial class frmMemberManagement : Form
    {
        ...
        public frmMemberManagement(IMemberService memberService, IAuthenticationService authenticationService, IMemberRepository memberRepository, IOptions<List<MemberObject>> members, frmMemberDetails frmMemberDetails)
        {
        ...
        {
            mtxtMemberID.Text = string.Empty;
        ...
        private MemberObject GetMemberObject()
        {
            ...
            try
            {
                ...
                {
                    MemberID = int.Parse(mtxtMemberID.Text),
        ...
        public void LoadMemberList()
        {
            if (_authenticationService.IsAdmin())
            {
                ...
                {
                    _memberRepository.CreateMember(sysMem);
                }
            }
            var members = _memberRepository.ViewMembers().Result.ToList();
            members = members.OrderBy(x => x.MemberID).ToList();
            try
            {
                source = new BindingSource();
                source.DataSource = members;
                mtxtMemberID.DataBindings.Clear();
                ...
                mtxtMemberID.DataBindings.Add("Text", source, "MemberID");
        ...
        private void dgvMemberList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            _frmMemberDetail.MemberId = ((MemberObject)source[e.RowIndex]).MemberID;
            _frmMemberDetail.FrmMemberManagement = this;
            if (_frmMemberDetail.ShowDialog() == DialogResult.OK)
            {
                //LoadMemberList();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = _memberRepository.DeleteMember(member.MemberID).Result;
                if(result == 1)
                {
                    LoadMemberList();
                }
        ...
        public void SearchMemberList(string? keyword, SearchBy searchBy)
        {
            ...
            var members = _memberRepository.SearchMembers(keyword, searchBy).Result.ToList();
            members = members.OrderBy(x => x.MemberID).ToList();
            try
            {
                source = new BindingSource();
